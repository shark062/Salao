...
<div class="min-h-screen flex flex-col items-center justify-center p-4 relative">
  ...
  <div class="w-full max-w-md">
    ...
    <div class="text-center mb-8">
      <h1 class="text-4xl font-bold text-indigo-600 dark:text-indigo-400 font-sans tracking-tight">ERIDES SOUZA ESTÚDIO</h1>
      <p class="text-slate-500 dark:text-slate-400">Sistema de Agendamento Online</p>
    </div>

    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 shadow-lg dark:shadow-2xl dark:shadow-black/50">
      <form (ngSubmit)="onLogin()" class="space-y-6">
        ...
        @if(error()) {
          <div class="bg-red-100 dark:bg-red-900/50 border border-red-300 dark:border-red-700 text-red-800 dark:text-red-300 px-4 py-2 rounded-lg text-sm">
            {{ error() }}
          </div>
        }

        <div>
          <button type="submit" class="w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-md text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-100 dark:focus:ring-offset-slate-900 focus:ring-indigo-500 transition-all duration-200 ease-in-out hover:scale-105 active:scale-100">
            Entrar
          </button>
        </div>
      </form>
